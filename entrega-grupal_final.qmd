...
author: "Filip Balik (NIE: X9509235T), Daniel García Rodriguez de Sepúlveda (DNI: 70829427F)"
format:
  html:
    theme: [style.scss]
    toc: true
...
#INTERACCION
gg <- ggplot(votos_elecciones, aes(x = fecha, y = votos, color = Grupo)) +
  geom_line(size = 1.2) +
  labs(
    title = "Votos por tipo de partido",
    x = "Fecha",
    y = "Votos",
...
datos_elecciones_tidy |> 
  ...
  slice_max(votos, n = 1) |> 
  select(fecha, municipio, votos, siglas, censo) |> 
  ggplot(aes(x =siglas, y = votos, fill = siglas)) +
  ...
  labs(
    title = "Partido político ganador en municipios con más de 100.000 habitantes",
    x = "Municipio",
    y = "Votos"
  )
```
